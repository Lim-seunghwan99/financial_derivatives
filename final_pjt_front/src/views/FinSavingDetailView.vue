<template>
  <div class="container">
    ...
    <div v-if="finlife">
      ...
      <hr />
      <div v-if="finlife.savingoptions_set">
        <div v-for="option in finlife.savingoptions_set">
          <p>저축금리유형 : {{ option.intr_rate_type }}</p>
          <p>저축금리유형명 : {{ option.intr_rate_type_nm }}</p>
          <p>적립유형명 : {{ option.rsrv_type_nm }}</p>
...
onMounted(() => {
  axios({
    method: "get",
    url: `${store.API_URL}/finlife/saving-products/${route.params.fin_prdt_cd}/`,
  })
    .then((res) => {
