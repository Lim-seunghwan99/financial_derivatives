...
def save_deposit_products(request):
    ...
    for deposit_option in deposit_option_list:
        ...
        if deposit_option_serializer.is_valid(raise_exception=True):
            deposit_product = DepositProduct.objects.get(fin_prdt_cd=deposit_option['fin_prdt_cd'])
    ...
    for saving_option in saving_option_list:
        ...
        if saving_option_serializer.is_valid(raise_exception=True):
            saving_product = SavingProduct.objects.get(fin_prdt_cd=saving_option['fin_prdt_cd'])
...
# 특정 예금 상품의 옵션 리스트 반환
@api_view(["GET"])
def deposit_product_option(request, fin_prdt_cd):
...
# 특정 적금 상품의 옵션 리스트 반환
@api_view(['GET'])
def saving_product_option(request, fin_prdt_cd):
