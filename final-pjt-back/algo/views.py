...
class UserProfileAPIView(APIView):
    ...
    def recommend_products(self, username):
        ...
        weight_age = 1.5
        weight_money = 2.0
        user_data = np.array([[user.age * 1000000, user.money * 10, user.salary]])
        user_data = user_data.reshape(1, -1)

        with open("../final-pjt-back/algo/user_data.json", "r", encoding='UTF8') as f:
            data = json.load(f)
        all_users = data
        all_users_data = np.array([[u['fields']['age'] * 1000000, u['fields']['money'] * 10, u['fields']['salary']] for u in all_users])
        ...
        for neighbor_data in nearest_neighbors_data:
            ...
            for user_entry in all_users:
                if (
                    user_entry['fields']['age'] == int(neighbor_age // 1000000)
                    and user_entry['fields']['money'] == int(neighbor_money // 10)
